...
function LiquidGradient({
  ...
  const containerClass = `liquid-container ${fullPage ? "full-page" : ""}`.trim();
  ...
  return (
    ...
    >
      ...
      {!hideUi && (
        <>
          <div ref={cursorRef} className={`cursor-ring ${isDarkMode ? "dark" : ""}`} style={{ opacity: showCursor ? 1 : 0 }} />
          <div
            ref={cursorDotRef}
          ...
          <button className={`cta-btn ${isDarkMode ? "dark" : ""}`} onClick={onCtaClick}>
            {ctaText}
          </button>
